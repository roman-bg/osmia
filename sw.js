...
// Инсталиране на Service Worker - кешира основните ресурси
...
self.addEventListener('fetch', (event) => {
    ...
    // Пропускаме несъществените заявки (аналитика, etc.)
    ...
    if (event.request.mode === 'navigate') {
        event.respondWith(
            fetch(event.request)
            .then((response) => {
                // Ако заявката е успешна, кешираме отговора за следващия път
    ...
    if (event.request.destination === 'style' ||
        ...
        event.respondWith(
            ...
            .then((cachedResponse) => {
                if (cachedResponse) {
                    // Върни от кеша, но обнови на заден план
                    fetch(event.request)
...
// Синхронизация на фона (ако е необходимо)
self.addEventListener('sync', (event) => {
    console.log('[SW] Background sync triggered');
    // Тук може да добавите логика за синхронизация на данни
});
